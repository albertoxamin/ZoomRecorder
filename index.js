...
const { email, password, meeting_url } = require('./credentials.js');
...
const main = async () => {
	...
	let code
	try {
		await loginPage.goto(meeting_url, { waitUntil: 'networkidle2' })
		...
		loginPage.close();
		await page.goto(meeting_url, { waitUntil: 'networkidle0' })

		try {
			await page.click('#joinBtn');
		} catch (e) { console.log('join not found') }
		...
		while (numParticipants !== '1') {
			await page.waitFor(5000)
			try {
				await page.click('div.zm-modal-footer > div > div > button')
			} catch (e) { }
			numParticipants = await page.$eval('.footer-button__number-counter > span', e => e.innerHTML)
			console.log(numParticipants)
		}
